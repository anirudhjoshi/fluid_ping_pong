(function () {
	...
	function Pong(canvas) {
		...
		this.updatePlayer = function () {
			...
			if ( ( player.y < 0 && player.vy < 0 ) || ( player.y + player.height > this.ctx.canvas.height && player.vy > 0 ) ) {
		...
		window.addEventListener("keydown", function(e) {
			...
			for(i in this.keyMap) {
				...
				if (this.keyMap.hasOwnProperty(i)) {
				
					if( e.keyCode === this.keyMap[i].code ) {
					
							this.keyMap[i].on = true;
							break;
	...
	var p = new Pong(document.getElementbyId("canvas"));
	p.theta = 5;
